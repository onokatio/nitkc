...
def ASK(data):
    t = np.arange(N)
    fc = 4
    return A * np.sin(2*np.pi*( (fc*t) /128)) * data
...
def FSK(data):
    ...
    f1 = 4
    if data == 0:
        return A * np.sin(2*np.pi*( (f0*t) /128))
    else:
        return A * np.sin(2*np.pi*( (f1*t) /128))
...
for bit,seq  in zip(bits,sequence):
    wave_ask.extend( ASK(bit) )
    wave_fsk.extend( FSK(bit) )
    wave_psk.extend( PSK(bit) )

plt.plot(np.arange(len(wave_ask)), wave_ask)
plt.plot(np.arange(len(wave_fsk)), wave_fsk)
